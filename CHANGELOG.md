...
#### 데이터 보존
- **미저장 파일 편집 영속화** — 열린 파일 버퍼(편집 내용·dirty 상태·활성 파일)를 `chrome.storage.local`(`devtoolsPpOpenFiles`)에 저장. 패널 reload / Service Worker 재활용에도 미저장 편집이 보존되고 마지막 활성 파일로 복귀. 이전엔 메모리에만 있어 reload 시 소실됐음
- **외부 변경 감지 (TOCTOU 가드)** — 파일 열 때 `lastModified` 기록 → 저장 직전 디스크 현재 mtime과 비교. 연 이후 외부(VS Code·git 등)에서 바뀌었으면 덮어쓰기 전 confirm, 취소 가능. 저장 후 mtime 갱신

#### 안전
- **`show all files` 토글 제거** — 텍스트 확장자 파일만 열람하도록 고정. 바이너리 파일을 열어 저장 시 발생하던 손상 경로 차단 (v0.18.0에서 도입했던 토글)
- **대용량 파일 경고** — 5MB 초과 파일은 열기 전 성능 경고 confirm
- **디렉토리 연결 일회성 경고** — 최초 연결 시 "영속 read/write 권한 부여 / 민감 폴더 연결 주의" 안내
- 권한 조회 실패 시 fail-closed(`prompt`) 처리

#### 버그 수정
- 트리 노드 펼치기 실패 시 토글 상태가 복원되지 않고 re-render가 누락되던 문제 수정
- 같은 폴더 중복 연결 방지 (`isSameEntry`)
- 미사용 함수(`_activeEntity`) 제거
