...
[**Chrome 웹 스토어에서 DevTools++ 설치**](https://chromewebstore.google.com/detail/devtools++/hnakhjcfhlpcjmdgpjaadobiefgkaaal)
...
2. 해당 요청 행 클릭 → Message 탭에서 응답 크기와 본문 확인
   ...
   - 응답이 정상 크기면 → 다음 단계
3. 같은 행의 JS Context 탭 → ±2초 윈도우의 JS 호출 확인. 별도 API 호출, 입력값 추출, 암호화 연산 등이 보이면 그게 클라이언트 측 판정 경로
...
4. 더 깊이 변조하려면 Intercept ON → 같은 요청 재트리거 → 요청 홀딩 → raw HTTP 에디터에서 자유 변조 → Forward Modified
